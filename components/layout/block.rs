...
use model::{min_size, specified, specified_or_none};
use sequential;
...
use style::values::computed::{LengthOrPercentageOrNone, LengthOrPercentageOrAuto, MinSize};
use util::clamp;
...
impl CandidateBSizeIterator {
    ...
    pub fn new(fragment: &Fragment, block_container_block_size: Option<Au>)
        ...
        let min_block_size = match (fragment.style.min_block_size(), block_container_block_size) {
            (MinSize::Percentage(percent), Some(block_container_block_size)) => {
                block_container_block_size.scale_by(percent)
            }
            (MinSize::Calc(calc), Some(block_container_block_size)) => {
                calc.length() + block_container_block_size.scale_by(calc.percentage())
            }
            (MinSize::Calc(calc), None) => calc.length(),
            (MinSize::Percentage(_), None) => Au(0),
            (MinSize::Length(length), _) => length,
            (MinSize::Auto, _) => Au(0),
        };
...
pub trait ISizeAndMarginsComputer {
    ...
    fn compute_used_inline_size(&self,
        ...
        // for 'inline-size'.
        let computed_min_inline_size = min_size(block.fragment().style().min_inline_size(),
                                                 containing_block_inline_size);
