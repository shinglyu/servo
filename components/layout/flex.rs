...
use model::{min_size, specified_or_none};
use std::cmp::{max, min};
use std::ops::Range;
use std::sync::Arc;
...
use style::values::computed::{LengthOrPercentageOrAuto, MinSize};
...
impl AxisSize {
    ...
    pub fn new(size: LengthOrPercentageOrAuto, content_size: Option<Au>,
               min: MinSize, max: LengthOrPercentageOrNone) -> AxisSize {
        match size {
    ...
    pub fn init_sizes(&mut self, flow: &mut Flow, containing_length: Au, direction: Direction) {
        ...
        match direction {
            ...
            Direction::Inline => {
                ...
                self.min_size = min_size(block.fragment.style.min_inline_size(), containing_length);
            }
            Direction::Block => {
                let basis = from_flex_basis(block.fragment.style.get_position().flex_basis,
                ...
                self.max_size = specified_or_none(block.fragment.style.max_block_size(),
                                                  containing_length).unwrap_or(MAX_AU);
                self.min_size = min_size(block.fragment.style.min_block_size(), containing_length);
            }
        }
    }
